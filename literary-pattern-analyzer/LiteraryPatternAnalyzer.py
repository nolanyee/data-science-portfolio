...
generated. As an example, the pre-processing procedure is performed for biblical books due the the relatively straight
forward structure of the website.
'''
...
def bible(bookname,startchapter,endchapter):
    ...
    global tokencount
    global book
    for i in range(int(startchapter), int(endchapter)): # Verse separation procedure depends on html structure
...
def topicmodel(book,window,generalize=1,numpass=2,filtlow=2,filthigh=0.8,filtmax=1000):
    ...
    LDAmodel = gensim.models.LdaModel(bowtfidf,num_topics=verses//(window*generalize),id2word=dictionary,passes=numpass)
...
if __name__ == '__main__':
    print('The following is a demonstration of the literary analysis tools using biblical books as the example texts.')
