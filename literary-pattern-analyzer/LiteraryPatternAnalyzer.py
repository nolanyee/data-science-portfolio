...
# Loading a Text File
def localtext(path, split = 'sentence'):
    global verselist
    global tokencount
    global book
    ...
    if split == 'sentence':
        ...
        for i in range(len(tokens)):
            versetoken.append(tokens[i])
            if tokens[i] == '.':
                versecounter += 1
                verselist.append([tokencount,str(versecounter)])
                versetoken = [str(versecounter)+' ']+versetoken
                outputtokens = outputtokens + versetoken
                versetoken = []
                tokencount = i+1
        outputtext = ' '.join(outputtokens)
        ...
        for i in range(len(tokens)):
            versetoken.append(tokens[i])
            if i%split ==0:
                verselist.append([tokencount, str(i//split+1)])
                versetoken = [str(i//split+1) + ' '] + versetoken
                outputtokens = outputtokens + versetoken
                versetoken = []
                tokencount = i + 1
        outputtext = ' '.join(outputtokens)
        ...
        tokens = nltk.word_tokenize(text)
        for i in range(len(tokens)):
            if re.search(split,tokens[i]):
                verselist.append([tokencount,tokens[i]])
            else:
                outputtokens.append(tokens[i])
                tokencount += 1
        book = outputtokens
...
if __name__ == '__main__':
    print('The following is a demonstration of the literary analysis tools using the Iliad or'+
          ' biblical books as the example texts. Enter "file" to analyze a text file.')
    ...
    if biblebook == 'Iliad':
        ...
        splittype = input("Split Type ('sentence', integer, or regex): ")
        localtext(filepath, splittype)
    else:
