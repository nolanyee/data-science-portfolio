...
headinglabel5.grid(row=26, column=0, sticky=E)

parameterfield('sobel_smooth_factor','Sobel Map Smoothing Factor',5,0,27)
parameterfield('sobel_neighbors_threshold','Sobel Magnitude Threshold',5,0.0,28)

headinglabel6 = Label(frame, text='DISPLACEMENT MAP PROCESSING')
headinglabel6.grid(row=29, column=0, sticky=E)

parameterfield('displace_threshold','Displacement Magnitude Threshold',5,10,30)
parameterfield('smooth_threshold','Sobel Magnitude Threshold',5,0.01,31)
parameterfield('smooth_factor','Displacement Smoothing Factor',5,2,32)

headinglabel6 = Label(frame, text='DEPTH MAP GENERATION')
headinglabel6.grid(row=33, column=0, sticky=E)
...
integration_algorithmlabel.grid(row=34, column=0, sticky=E)
integration_algorithmcombo.grid(row=34, column=1, sticky=W)

parameterfield('integration_fold','Number of Angles for Rotation',5,90,35)
parameterfield('initial_iterations','Initial Iterations',5,10,36)
parameterfield('final_iterations','Final Iterations',5,200,37)
...
bias_complexitylabel.grid(row=38, column=0, sticky=E)
bias_complexitycombo.grid(row=38, column=1, sticky=W)

parameterfield('final_smooth_threshold','Smoothing Sobel Magnitude Threshold',5,0.5,39)
parameterfield('final_smooth_factor','Sobel Masked Smoothing Factor',5,1.6,40)
parameterfield('full_smooth_factor','Overall Smoothing Factor',5,0.5,41)

parameterfield('normaloutput','Normal Map Output File Path',80,'',42)
parameterfield('depthoutput','Depth Map Output File Path',80,'',43)
...
okbutton.grid(row=44, column = 1, sticky = W)
...
# Constants
if cast_shadow:
    if secondary_light == 'ambient':
        shadowmin = np.max(image[shadowmask == 1])
        shadowslope = min(shadow[0]) - shadowmin
    else:
        shadowmin = min(shadow[0])
        shadowslope = np.max(image[shadowmask == 1]) - shadowmin
else:
    shadowmin = min(shadow[0])
    shadowslope = -1*min(shadow[0])
...
# Truncating gradient
cospsi = (image-shadowmin)/shadowslope
...
shadowadjustimage = np.where((image>shadowmaximum-shadow_max_offset)&(image<shadowmaximum), shadowmaximum,
                             shadowadjustimage)
shadowadjustimage = np.where((image<shadowmaximum-shadow_max_offset)&(image>shadowmin+shadow_min_offset),
                             shadowmin+(shadowmaximum-shadowmin)/
                             (shadowmaximum-shadow_max_offset-shadowmin-shadow_min_offset)*
                             (shadowadjustimage-shadowmin-shadow_min_offset),
                             shadowadjustimage)

# Calculating normal vector
shadownx =np.where(dpdx==0,0,(shadowadjustimage-shadowmin)/(shadowslope*(sinshadowtheta+cosshadowtheta*shadowdpdydpdx)))
shadowny =np.where(dpdy==0,0,(shadowadjustimage-shadowmin)/(shadowslope*(sinshadowtheta*shadowdpdxdpdy+cosshadowtheta)))
